...
@router.callback_query(F.data == 'AppleMac')
async def show_gaming_pc_with_cart(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"Пользователь: {callback.from_user.first_name} выбрал раздел Моноблоки/Apple")

    products = await get_subcategory_products("моноблоки", "apple_imac")  # Получаем товары

    if not products:
        await callback.answer("❌ В подкатегории 'Моноблоки/Apple' пока нет товаров.")
        return
