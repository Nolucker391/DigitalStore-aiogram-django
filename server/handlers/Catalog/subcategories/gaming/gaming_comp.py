...
async def show_gaming_pc_with_cart(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"Пользователь: {callback.from_user.first_name} выбрал раздел Компьютеры/Игровые")
...
async def update_product_message(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not products:
        ...
        return
    # '/app/api/uploads/products/компьютеры/игровые/product_1/logo_bRzVsTH.png'
    # / Users / skillbox / PycharmProjects / ai / api / uploads / products / компьютеры / игровые / product_1 / Снимок_экрана_2025 - 03 - 04_в_14.38 .03.png
