...
@router.callback_query(F.data == 'workpc')
async def show_gaming_pc_with_cart(callback: types.CallbackQuery, state: FSMContext):
    """ Показывает товары из подкатегории 'компьютеры для работы', если она принадлежит категории 'Компьютеры' """
    await set_user_state(state, UserState.select_office_pc)
    logger.info(f"Пользователь: {callback.from_user.first_name} выбрал раздел Компьютеры/Для работы")

    products = await get_subcategory_products("компьютеры", "для работы")  # Получаем товары

    if not products:
        await callback.answer("❌ В подкатегории 'Компьютеры для работы' пока нет товаров.")
        return
