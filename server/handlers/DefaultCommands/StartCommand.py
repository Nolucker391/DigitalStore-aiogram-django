...
async def start_command(message_or_callback: types.Message | types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"🔍 Данные пользователя: ID: {user_id}, Username: {username}, First Name: {first_name}")
    ...
    if user_db:
        if created:
            logger.info(f"✅ Новый пользователь зарегистрирован: {user_db}")
        else:
            logger.info(f"ℹ️ Пользователь уже существует: {user_db}")
    else:
        logger.ERROR(f"❌ Ошибка при добавлении пользователя {user_id}")
