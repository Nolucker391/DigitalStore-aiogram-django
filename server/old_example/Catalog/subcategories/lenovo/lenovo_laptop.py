...
@router.callback_query(F.data == 'Lenovo')
async def show(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"Пользователь: {callback.from_user.first_name} выбрал раздел Ноутбуки/Lenovo")

    products = await get_subcategory_products("ноутбуки", "lenovo")  # Получаем товары
