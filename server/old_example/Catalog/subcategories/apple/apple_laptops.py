...
async def show(callback: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"Пользователь: {callback.from_user.first_name} выбрал раздел Ноутбуки/Apple")

    products = await get_subcategory_products("ноутбуки", "apple")  # Получаем товары
